  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <nav className="ml-auto hidden items-center gap-0.5 lg:flex">
            {NAV.map((item) => (
              <Link
                ...
                to={item.to}
                preload="intent"
                className="rounded-full px-4 py-2 text-sm font-semibold text-muted-foreground transition-all hover:bg-foreground hover:!text-[var(--background)]"
        ...
        <nav className="flex gap-0.5 overflow-x-auto border-t border-border px-5 py-2 lg:hidden">
          {NAV.map((item) => (
            <Link
              ...
              to={item.to}
              preload="intent"
              className="shrink-0 rounded-full px-4 py-2 text-sm font-semibold text-muted-foreground transition-all hover:bg-foreground hover:!text-[var(--background)]"
              activeProps={{ className: "bg-foreground !text-[var(--background)] shadow-md hover:bg-foreground hover:!text-[var(--background)]" }}
              activeOptions={{ exact: item.to === "/" }}
            >
              {item.label}
      ...
      <main className="mx-auto max-w-[1400px] px-5 py-8">
        ...
        <div key={router.state.location.pathname} className="animate-in fade-in slide-in-from-bottom-2 duration-150">
          {children}
        </div>
      </main>
    </div>
